...
| **V14.2** | Secret Management | Separates API keys and environment-specific configs from code. | Credentials are supplied through GitHub Actions and OpenShift secrets. |
...
| **V14.4** | Network Segmentation | Controls pod communication between frontend and backend tiers. | Hardened [NetworkPolicies](https://kubernetes.io/docs/concepts/services-networking/network-policies/) are applied by the platform. |
...
Trivy reports findings at every severity to the GitHub Security tab on each run, where they are triaged against the windows above. Findings are not a merge gate, because advisories are published independently of any contribution. Committed secrets are blocked at push time by GitHub secret scanning push protection instead.
